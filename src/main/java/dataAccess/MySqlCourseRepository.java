...
public class MySqlCourseRepository implements MyCourseRepository {
    ...
    //Nimmt ein Course-Entity entgegen dass noch keine id hat, schreibt es in die DB,
    //holt es aus der DB wieder raus mit einer id, und gibt die fertige Entity
    //wieder zurück an den Aufrufer des Insert-Statements
    @Override
    public Optional<Course> insert(Course entity) {
        Assert.notNull(entity); //der Wert darf nicht null sein
        try {
            String sql = "INSERT INTO `courses` (`name`, `description`, `hours`, `begindate`, `enddate`, `coursetype`) " +
                    "VALUES (?,?,?,?,?,?)";
    ...
    public List<Course> getAll() {
        ...
        try {
            ...
            //liefert das Ergebnis von der executeQuery
    ...
    public Optional<Course> update(Course entity) {
        ...
        String sql = "UPDATE `courses` SET `name` = ?, `description` = ?, `hours` = ?, " +
                "`begindate` = ?, `enddate` = ?, `coursetype` = ? WHERE `courses`.`id` = ?";
    ...
    public List<Course> findAllCourseByNameOrDescription(String searchText) {
        try {
            String sql = "SELECT * FROM `courses` WHERE LOWER(`description`) " +
                    "LIKE LOWER(?) OR LOWER(`name`) LIKE LOWER(?)";
            PreparedStatement preparedStatement = con.prepareStatement(sql);

            // %-Zeichen in der SQL bedeutet dass alles an dieser Stelle in der Suche angenommen wird
